...
int main(int, char**)
{
    username == NULL; pass == NULL; key == NULL;
    RECT desktop;
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        ImGui::SetNextWindowPos(ImVec2(0, 0), ImGuiCond_Once);
        {
            ...
            {
                if (ImGui::BeginPopupContextItem(skCrypt("Credits")))
                {
                    ImGui::Separator();
                    ImGui::Text(skCrypt(" ImGui from Ocornut\n Special thanks to KleskBY, LKS & Sprtx\n Develop by Nova644"));
                    ImGui::Separator();
                    ImGui::EndPopup();
                }
                if (ImGui::BeginPopupContextItem(skCrypt("Settings")))
                {
                    ImGui::Separator();

                    ImGui::ShowStyleSelector(skCrypt(" Colors"));

                    ImGui::Separator();
                    if (ImGui::Button(skCrypt("    Close    "), ImVec2(186, 29)))
                    {
                ...
                {
                    ImGui::Separator();
                    ImGui::NewLine(); ImGui::Text(skCrypt("  ")); ImGui::SameLine();
                    ImGui::InputText(skCrypt("  :  Username"), username, IM_ARRAYSIZE(username));
                    ImGui::Text(skCrypt("  ")); ImGui::SameLine();
                    ImGui::InputText(skCrypt("  :  Password"), pass, IM_ARRAYSIZE(pass), ImGuiInputTextFlags_Password);
                    ImGui::Text(skCrypt("  ")); ImGui::SameLine();
                    ImGui::InputText(skCrypt("  :  Key"), key, IM_ARRAYSIZE(key));
                    ImGui::Text(skCrypt("      ")); ImGui::SameLine();
                    ImGui::Checkbox(skCrypt(" Accept Terms & Conditions"), &terms);
                    ImGui::NewLine(); ImGui::NewLine();
                    ImGui::Separator();
                    ImGui::NewLine();
                    if (ImGui::Button(skCrypt("    Login    "), ImVec2(186, 29)))
                    {
                        if (username[0] == '\0' || pass[0] == '\0')//check if username and password is empty
                        {
                            MessageBox(hwnd, skCrypt("An error has occurred please check \nyour username or password !"), skCrypt("NovaGUI"), MB_ICONEXCLAMATION);
                        }
                        else
                        {
                            if (terms == true)
                ...
                if (currentColumn == 2)
                {
                    ...
                    ImGui::NewLine(); ImGui::NewLine(); ImGui::NewLine(); ImGui::NewLine(); ImGui::Separator(); ImGui::NewLine();
                    if (ImGui::Button(skCrypt("    Logout    "), ImVec2(186, 29)))
                    {
                        currentColumn = 0;
                    }
                    ImGui::SameLine();
                    if (ImGui::Button(skCrypt("    Settings    "), ImVec2(186, 29)))
                    {
                        ImGui::OpenPopup(skCrypt("Settings"));
                    }
                    ImGui::SameLine();
                    if (ImGui::Button(skCrypt("    Exit    "), ImVec2(186, 29)))
                    {
